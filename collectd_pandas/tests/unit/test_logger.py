...
from collectd_pandas.tests.match import wildcard
...
class TestCollectdLogHandler(unittest.TestCase):
    ...
    @staticmethod
    def test_verbose_debug_when_emit():
        # Given
        collectd = mock_collectd()
        handler = CollectdLogHandler(collectd=collectd)
...
def mock_collectd(*args, **kwargs):
    return MagicMock(spec=MockCollectd, *args, **kwargs)


class MockCollectd(object):
